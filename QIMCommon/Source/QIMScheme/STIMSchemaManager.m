...
@interface STIMSchemaManager ()

@property (nonatomic, strong) NSMutableSet *schemas;

@end

@implementation STIMSchemaManager
...
static STIMSchemaManager *__manager = nil;
+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __manager = [[STIMSchemaManager alloc] init];
...
- (void)postSchemaNotificationWithUrl:(NSURL *)url {
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"STIMSchemaNotification" object:url];
    });
}
