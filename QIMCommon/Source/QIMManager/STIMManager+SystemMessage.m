...
@implementation STIMManager (SystemMessage)

#pragma mark - 系统消息

- (void)checkHeadlineMsg {
    long long errorTime = [[[STIMUserCacheManager sharedInstance] userObjectForKey:kGetNewSystemHistoryMsgError] longLongValue];
    STIMVerboseLog(@"检查本地是否有系统消息错误时间戳 : %lld", errorTime);
    if (errorTime > 0) {
...
- (void)updateLastSystemMsgTime {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        STIMVerboseLog(@"更新本地HeadLine消息时间戳");
        ...
        if (errorTime > 0) {
            ...
            STIMVerboseLog(@"本地HeadLine消息错误时间戳 : %lld", errorTime);
        } else {
            self.lastSystemMsgTime = [[IMDataManager stIMDB_SharedInstance] stIMDB_lastestSystemMessageTime];
        }
        if (self.lastSystemMsgTime == 0) {
            self.lastSystemMsgTime = defaultTime;
        }
        STIMVerboseLog(@"强制塞本地HeadLine消息时间戳到为 kGetNewSystemHistoryMsgError : %f", self.lastSystemMsgTime);
        [[STIMUserCacheManager sharedInstance] setUserObject:@(self.lastSystemMsgTime) forKey:kGetNewSystemHistoryMsgError];
        STIMVerboseLog(@"强制塞本地HeadLine消息时间戳到为 kGetNewSystemHistoryMsgError : %f完成", self.lastSystemMsgTime);
        STIMVerboseLog(@"强制塞本地HeadLine消息消息时间戳完成之后再取一下本地错误时间戳 : %lld", [[[STIMUserCacheManager sharedInstance] userObjectForKey:kGetNewSystemHistoryMsgError] longLongValue]);

        STIMVerboseLog(@"最终获取到的本地HeadLine最后消息时间戳为 : %lf", self.lastSystemMsgTime);
    });
}
...
- (void)updateOfflineSystemNoticeMessages {
    ...
    [self getSystemMsgHistoryListWithUserId:from WithDomain:[[STIMManager sharedInstance] getDomain] WithVersion:self.lastSingleMsgTime];
}
...
- (void)getSystemMsgLisByUserId:(NSString *)userId WithFromHost:(NSString *)fromHost WithLimit:(int)limit WithOffset:(int)offset withLoadMore:(BOOL)loadMore WithComplete:(void (^)(NSArray *))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *array = [[IMDataManager stIMDB_SharedInstance] stIMDB_getMgsListBySessionId:userId WithRealJid:userId WithLimit:limit WithOffset:offset];
        if (array.count > 0) {
            ...
            if (list.count < limit && loadMore == YES) {
                ...
                dispatch_async(self.load_history_msg, ^{
                    ...
                    NSArray *result = [self getSystemMsgListWithDirection:0 WithUserId:userId WithFromHost:fromHost WithLimit:limit - list.count withTimeVersion:version toId:[STIMManager getLastUserName] toHost:fromHost];
                    if (result.count > 0) {
            ...
            if (loadMore == YES) {
                ...
                dispatch_async(self.load_history_msg, ^{
                    long long version = [[IMDataManager stIMDB_SharedInstance] stIMDB_getMinMsgTimeStampByXmppId:userId] - timeChange;
                    NSArray *resultList = [self getSystemMsgListWithDirection:0 WithUserId:userId WithFromHost:fromHost WithLimit:limit withTimeVersion:version toId:[STIMManager getLastUserName] toHost:fromHost];
                    if (resultList.count > 0) {
                        [[IMDataManager stIMDB_SharedInstance] stIMDB_bulkInsertHistoryChatJSONMsg:resultList];
                        NSArray *datas = [[IMDataManager stIMDB_SharedInstance] stIMDB_getMgsListBySessionId:userId WithRealJid:nil WithLimit:(int)(resultList.count) WithOffset:offset];
...
- (NSArray *)getSystemMsgListWithDirection:(int)direction WithUserId:(NSString *)userId WithFromHost:(NSString *)fromHost WithLimit:(NSInteger)limit withTimeVersion:(long long)version toId:(NSString *)toId toHost:(NSString *)toHost {
    ...
    NSString *destUrl = [NSString stringWithFormat:@"%@/qtapi/get_system_msgs.qunar?v=%@&p=iOS&u=%@&k=%@&f=t", [[STIMNavConfigManager sharedInstance] javaurl], [[STIMAppInfo sharedInstance] AppBuildVersion], [STIMManager getLastUserName], self.remoteKey];
...
- (void)getSystemMsgHistoryListWithUserId:(NSString *)userId WithDomain:(NSString *)domain WithVersion:(long long)version {
    ...
    [params setObject:@"t" forKey:@"f"];
    NSData *requestData = [[STIMJSONSerializer sharedInstance] serializeObject:params error:nil];
    NSString *destUrl = [NSString stringWithFormat:@"%@/qtapi/get_system_history.qunar?v=%@&p=iOS&u=%@&k=%@&f=t", [[STIMNavConfigManager sharedInstance] javaurl], [[STIMAppInfo sharedInstance] AppBuildVersion], [STIMManager getLastUserName], self.remoteKey];
    ...
    [STIMHTTPClient sendRequest:request complete:^(STIMHTTPResponse *response) {
        ...
        NSDictionary *logDic = @{@"costTime":@([[STIMWatchDog sharedInstance] escapedTimewithStartTime:startTime]), @"reportTime":@([[NSDate date] timeIntervalSince1970]), @"threadName":@"", @"isMainThread":@([NSThread isMainThread]), @"url":destUrl, @"methodParams":params, @"requestHeaders":requestHeaders, @"describtion":@"请求HeadLine离线消息"};
...
- (void)dealWithSystemMessages:(NSArray *)systemMsgs {
    ...
    long long lastMaxTime = [[IMDataManager stIMDB_SharedInstance] stIMDB_bulkInsertHistoryChatJSONMsg:systemMsgs];
