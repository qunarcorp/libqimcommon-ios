...
@implementation STIMManager (Helper)
...
- (void)playSound {
    ...
    if ([self isNewMsgNotify]) {
        ...
        if ([[NSDate date] timeIntervalSince1970] - lastPlay > 2.0) {
            UIApplicationState applicationState = [[UIApplication sharedApplication] applicationState];
            if (applicationState == UIApplicationStateActive) {
                NSString *soundName = [[STIMManager sharedInstance] soundName];
                if (!soundName) {
                    // 非租车业务才播放声音
                    SystemSoundID soundID;
                    // 读文件获取SoundID
...
- (BOOL)addSkipBackupAttributeToItemAtURL:(NSURL *)URL {
    if ([[NSFileManager defaultManager] fileExistsAtPath:[URL path]]) {
        ...
        if (!success) {
            STIMErrorLog(@"Error excluding %@ from backup %@", [URL lastPathComponent], error);
        }
        return success;
    }
    return NO;
