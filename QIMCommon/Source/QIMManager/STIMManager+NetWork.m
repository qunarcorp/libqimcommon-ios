...
@implementation STIMManager (NetWork)
...
- (BOOL)checkNetworkCanUser{
    NSString *checkUrl = [[STIMNavConfigManager sharedInstance] healthcheckUrl];
    STIMVerboseLog(@"网络检测，检测地址:%@...", checkUrl);
    if (checkUrl.length > 0) {
        ...
        if ([request responseStatusCode] == 200) {
            STIMWarnLog(@"<Method: checkNetworkCanUser> 网络检测，已连接到互联网...");
            return YES;
        } else {
            STIMWarnLog(@"网络检测，Request Url %@, Respone Code : %d , Error %@",checkUrl,request.responseStatusCode,request.error);
            return NO;
        }
    } else {
        return YES;
...
- (void)checkNetworkStatus{
    ...
    STIMWarnLog(@" _needTryRelogin = %d", self.needTryRelogin);
...
- (void)onNetworkChange:(NSNotification *)notify{
    ...
    switch ([notify.object intValue]) {
        ...
        case ReachableViaWiFi:
        {
            ...
            STIMVerboseLog(@"<手机连接到了Wi-Fi %@, 信号强度 = %@>", wifiName, wifiSignalStrength);
        }
            break;
        case ReachableViaWWAN: {
            STIMVerboseLog(@"<手机连接到了ViaWWAN网络>...");
            self.needTryRelogin = YES;
        }
            break;
        default:
        {
            STIMWarnLog(@"手机已连接到网络，但不一定能上网...");
            self.needTryRelogin = YES;
        }
            break;
    }
...
- (void)updateAppWorkState:(AppWorkState)appWorkState {
    ...
    if (oldAppWorkState != newAppWorkState) {
        ...
        STIMWarnLog(@"App工作状态即将变为 : %u", appWorkState);
        dispatch_async(dispatch_get_main_queue(), ^{
