//
//  STIMManager+XmppImManagerEvent.m
//  qunarChatIphone
//
...
@implementation STIMManager (XmppImManagerEvent)
...
//        STIMVerboseLog(@"登录之前数据库进行checkPoint");
//        [[IMDataManager stIMDB_SharedInstance] stIMDB_dbCheckpoint];
//        STIMVerboseLog(@"登录之前数据库完成checkPoint");
        
        STIMVerboseLog(@"登录之前初始化数据库文件之后更新各种时间戳开始 : %@", infoDic);
        ...
        STIMVerboseLog(@"登录之前初始化数据库文件之后更新各种时间戳完成");
    });
}

- (void)receiveErrorMessage:(NSDictionary *)infoDic {
    STIMVerboseLog(@"收到消息的错误回执 : %@", infoDic);
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        switch (errCode) {
            case STIMMessageErrCodeRefused: {
                if ([from containsString:@"conference."]) {
                    
                } else {
                   STIMMessageModel *msg = [[STIMManager sharedInstance] createMessageWithMsg:@"消息已发出，但被对方拒收了。" extenddInfo:nil userId:from realJid:from userType:ChatType_SingleChat msgType:STIMMessageType_Time forMsgId:[STIMUUIDTools UUID] willSave:YES];
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)receiveCategoryNotifyMessage:(NSDictionary *)msgDic {
    dispatch_async(self.receive_notify_queue, ^{
         ...
         */
        NSDictionary *notifyMsg = [[STIMJSONSerializer sharedInstance] deserializeObject:[msgDic objectForKey:@"bodyValue"] error:nil];
        STIMVerboseLog(@"收到通知类Presence : %@", msgDic);
        switch (categoryType) {
            case STIMCategoryNotifyMsgTypeOrganizational: {
                STIMVerboseLog(@"重新获取组织架构");
                dispatch_async(self.cacheQueue, ^{
                    [self updateOrganizationalStructure];
                });
            }
                break;
            case STIMCategoryNotifyMsgTypeSession: {
                STIMVerboseLog(@"打开了一个会话，本客户端暂不支持同步该类型");
            }
                break;
            case STIMCategoryNotifyMsgTypeNavigation: {
                long long navVersion = [[notifyMsg objectForKey:@"navversion"] longLongValue];
                if (navVersion > [[STIMNavConfigManager sharedInstance] navVersion]) {
                    [[STIMUserCacheManager sharedInstance] setUserObject:[STIMManager getLastUserName] forKey:@"currentLoginUserName"];
                    [[STIMNavConfigManager sharedInstance] qimNav_updateNavigationConfigWithCheck:YES];
                }
                STIMVerboseLog(@"下发导航版本通知");
            }
                break;
            case STIMCategoryNotifyMsgTypeOPSUnreadCount: {
                STIMVerboseLog(@"收到OPS 骆驼帮未读数更新通知");
                ...
                break;
            case STIMCategoryNotifyMsgTypePersonalConfig: {
                STIMVerboseLog(@"收到个人配置更新通知");
...
//                NSNumber *forceOldSearch = [notifyMsg objectForKey:@"forceOldSearch"];
                ...
                if ([[IMDataManager stIMDB_SharedInstance] stIMDB_getConfigVersion] < version && ![resource isEqualToString:[[XmppImManager sharedInstance] resource]]) {
                    [self getRemoteClientConfig];
                } else if (forceUpdate) {
                    STIMVerboseLog(@"强制更新个人配置信息");
                    [[IMDataManager stIMDB_SharedInstance] stIMDB_clearClientConfig];
                    [self getRemoteClientConfig];
                } else if (forceQuickReplyUpdate) {
                    STIMVerboseLog(@"强制更新快捷回复");
                    [[IMDataManager stIMDB_SharedInstance] stIMDB_clearQuickReplyGroup];
                    [[IMDataManager stIMDB_SharedInstance] stIMDB_clearQuickReplyContents];
                    [self getRemoteQuickReply];
                } else if (forceRNUpdate) {
                    STIMVerboseLog(@"收到RN包清除通知");
                    ...
                    if (removeSussess) {
                        STIMVerboseLog(@"清空RN包缓存成功 : %@", latestJSCodeURLString);
                    } else {
                        STIMVerboseLog(@"清空RN包缓存失败 : %@", latestJSCodeURLString);
                    }
                } else if (forceOldSearch) {
                    STIMVerboseLog(@"强制切换老版本搜索");
                    NSNumber *forceOldSearchNumber = [notifyMsg objectForKey:@"forceOldSearch"];
                    [[STIMUserCacheManager sharedInstance] setUserObject:forceOldSearchNumber forKey:@"forceOldSearch"];
                } else {
                    
                }
            }
                break;
            case STIMCategoryNotifyMsgTypeBigIM: {
                STIMVerboseLog(@"大客户端通知，可忽略");
            }
                break;
            case STIMCategoryNotifyMsgTypeCalendar: {
                STIMVerboseLog(@"收到日历同步通知");
                long long newVersion = [[notifyMsg objectForKey:@"updateTime"] longLongValue];
                long long oldVersion = [[[IMDataManager stIMDB_SharedInstance] stIMDB_getConfigInfoWithConfigKey:[self transformClientConfigKeyWithType:STIMClientConfigTypeKLocalTripUpdateTime] WithSubKey:[[STIMManager sharedInstance] getLastJid] WithDeleteFlag:NO] longLongValue];
                if (newVersion > oldVersion) {
                    STIMVerboseLog(@"本次服务器下发的版本号 大于本地版本号 %lld，更新远程数据", oldVersion);
                    [self getRemoteUserTripList];
                } else {
                    STIMVerboseLog(@"本次服务器下发的版本号 小于本地版本号 %lld，不同步数据", oldVersion);
                }
            }
                break;
            case STIMCategoryNotifyMsgTypeOnline: {
                STIMVerboseLog(@"收到其他客户端上线下线通知");
            ...
            case STIMCategoryNotifyMsgTypeTickUser: {
                ...
                break;
            case STIMCategoryNotifyMsgTypeGlobalNotification: {
                /*
                ...
                break;
            case STIMCategoryNotifyMsgTypeDesignatedNotification:{
                /*
                 {
                 "from":"shop323",
                 "to":"hubin.hu",
                 ...
                 */
                STIMVerboseLog(@"下发指定通知,故需要相关会话信息");
                ...
                break;
            case STIMCategoryNotifyMsgTypeHotLineSync:{
                [[STIMManager sharedInstance] getRemoteHotlineShopList];
            }
                break;
            case STIMCategoryNotifyMsgTypeTickUserWorkWorldNotice: {
                NSString *onlineListStr = [msgDic objectForKey:@"bodyValue"];
                NSDictionary *onlineDict = [[STIMJSONSerializer sharedInstance] deserializeObject:onlineListStr error:nil];
                [[IMDataManager stIMDB_SharedInstance] stIMDB_bulkinsertNoticeMessage:@[onlineDict]];
                NSInteger eventType = [[onlineDict objectForKey:@"eventType"] integerValue];
                if (eventType == STIMWorkFeedNotifyTypeComment) {
                    STIMVerboseLog(@"online Comment 通知 : %@", onlineDict);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        STIMVerboseLog(@"发送驼圈在线消息小红点通知数: %ld", notReadMessageCount);
                ...
                } else if (eventType == STIMWorkFeedNotifyTypePOST) {
                    STIMVerboseLog(@"online 新帖子 通知 : %@", onlineDict);
                    ...
                    if (![ownerId isEqualToString:[[STIMManager sharedInstance] getLastJid]]) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                ...
                } else if (eventType == STIMWorkFeedNotifyTypePOSTAt) {
                  //帖子艾特
                    STIMVerboseLog(@"online 新帖子艾特 通知 : %@", onlineDict);
                    STIMVerboseLog(@"online Comment 通知 : %@", onlineDict);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:kPBPresenceCategoryNotifyWorkNoticeMessage object:nil];
                        NSInteger notReadMessageCount = [[STIMManager sharedInstance] getWorkNoticeMessagesCountWithEventType:@[@(STIMWorkFeedNotifyTypeComment), @(STIMWorkFeedNotifyTypePOSTAt), @(STIMWorkFeedNotifyTypeCommentAt)]];
                ...
                } else if (eventType == STIMWorkFeedNotifyTypeCommentAt) {
                  //评论艾特
                    STIMVerboseLog(@"online 新评论艾特 通知 : %@", onlineDict);
                    STIMVerboseLog(@"online Comment 通知 : %@", onlineDict);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:kPBPresenceCategoryNotifyWorkNoticeMessage object:nil];
                        NSInteger notReadMessageCount = [[STIMManager sharedInstance] getWorkNoticeMessagesCountWithEventType:@[@(STIMWorkFeedNotifyTypeComment), @(STIMWorkFeedNotifyTypePOSTAt), @(STIMWorkFeedNotifyTypeCommentAt)]];
                    ...
                    STIMVerboseLog(@"online 驼圈其他通知 : %@", onlineDict);
                }
            }
                break;
            case STIMCategoryNotifyMsgTypeMedalListUpdateNotice: {
                STIMVerboseLog(@"online 更新勋章列表");
                NSInteger version = [[notifyMsg objectForKey:@"medalVersion"] integerValue];
                [[STIMManager sharedInstance] getRemoteMedalList];
            }
                break;
            case STIMCategoryNotifyMsgTypeUserMedalUpdateNotice: {
                STIMVerboseLog(@"online 更新用户的勋章");
                NSInteger version = [[notifyMsg objectForKey:@"userMedalVersion"] integerValue];
                [[STIMManager sharedInstance] getRemoteUserMedalListWithUserId:nil];
            }
                break;
            default: {
                STIMVerboseLog(@"遇到不认识的categoryType : %@", msgDic);
            }
                break;
        }
...
- (void)onRevoke:(NSDictionary *)infoDic {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[IMDataManager stIMDB_SharedInstance] stIMDB_revokeMessageByMsgId:msgId WithContent:msg WithMsgType:STIMMessageType_Revoke];
    });
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)receiveMsg:(NSDictionary *)msgDic {
    ...
    dispatch_async(self.receive_msg_queue, ^{
        ...
        if (msgType == STIMMessageType_shareLocation && direction == 1) {
            NSDictionary *shareDic = [[STIMJSONSerializer sharedInstance] deserializeObject:extendInfo error:nil];
        ...
        if (msgType == STIMMessageType_RedPackInfo || msgType == STIMMessageType_AAInfo) {
            NSDictionary *redPackInfoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:extendInfo error:nil];
            NSString *jid = [redPackInfoDic objectForKey:@"From_User"];
            if ([jid isEqualToString:[STIMManager getLastUserName]] == NO) {
                return;
            }
        }
        
        if ([[IMDataManager stIMDB_SharedInstance] stIMDB_checkMsgId:msgId]) {
            return;
        }
        ...
        [mesg setMessageDirection:direction == 2 ? STIMMessageDirection_Sent : STIMMessageDirection_Received];
        [mesg setMessage:msg];
        [mesg setMessageSendState:STIMMessageSendState_Success];
        [mesg setMessageReadState:STIMMessageRemoteReadStateDidSent];
        [mesg setMessageDate:msgDate];
        [mesg setExtendInformation:extendInfo];
        [mesg setTo:[[STIMManager sharedInstance] getLastJid]];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (![sid isEqualToString:self.currentSessionUserId] && mesg.messageDirection == STIMMessageDirection_Received) {
                if (mesg.messageType == STIMMessageType_RedPack) {
...
- (void)receiveGroupMsg:(NSDictionary *)msgDic {
    dispatch_async(self.receive_msg_queue, ^{
        ...
        if (msgType == STIMMessageType_shareLocation && [self getGroupMsgDirectionWithSendJid:sendJid] == STIMMessageDirection_Received) {
            NSDictionary *shareDic = [[STIMJSONSerializer sharedInstance] deserializeObject:extendInfo error:nil];
            NSString *shareId = [shareDic objectForKey:@"shareId"];
            NSString *fromUser = [shareDic objectForKey:@"fromId"];
            [self.shareLocationDic setObject:shareId forKey:sid];
        ...
        if (msgType == STIMMessageType_RedPackInfo || msgType == STIMMessageType_AAInfo) {
            ...
            if ([fid isEqualToString:[STIMManager getLastUserName]] == NO) {
                return;
            }
        }
        // 消息已存在
        if ([[IMDataManager stIMDB_SharedInstance] stIMDB_checkMsgId:messageId]) {
            return;
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (mesg.messageDirection == STIMMessageDirection_Received && (![self.currentSessionUserId isEqualToString:sid])) {
                if (mesg.messageType == STIMMessageType_NewAt) {
                    //新版艾特消息
                    ...
                    if ([array isKindOfClass:[NSArray class]]) {
                        ...
                        for (NSDictionary *someOneAtDic in [groupAtDic objectForKey:@"data"]) {
                            ...
                            if ([someOneJid isEqualToString:[[STIMManager sharedInstance] getLastJid]]) {
                                atMe = YES;
                            }
                        }
                    }
                    if (atMe == YES) {
                        [self addAtMeMessageByJid:sid withType:STIMAtTypeSP withMsgId:mesg.messageId withMsgTime:[mesg messageDate]];
                    }
                } else {
                    if ([msg rangeOfString:@"@"].location != NSNotFound) {
                        ...
                        if (hasAtAll) {
                            [self addAtMeMessageByJid:sid withType:STIMAtTypeALL withMsgId:mesg.messageId withMsgTime:[mesg messageDate]];
                        }
                        if (hasAt) {
                            [self addAtMeMessageByJid:sid withType:STIMAtTypeSP withMsgId:mesg.messageId withMsgTime:[mesg messageDate]];
                        }
...
- (void)receiveSystemMsg:(NSDictionary *)msgDic {
    STIMVerboseLog(@"在线收到系统消息: %@", msgDic);
    dispatch_async(self.receive_msg_queue, ^{
        ...
        NSString *msgRaw = [msgDic objectForKey:@"msgRaw"];
        long long msgDate = [[msgDic objectForKey:@"stamp"] timeIntervalSince1970] * 1000;
        NSString *sid = [NSString stringWithFormat:@"SystemMessage@%@", [[STIMNavConfigManager sharedInstance] domain]];
        NSString *autoReply = [msgDic objectForKey:@"autoReply"];
        ...
        [mesg setMessageDirection:STIMMessageDirection_Received];
        [mesg setMessage:msg];
        [mesg setMessageDate:msgDate];
        [mesg setMsgRaw:msgRaw];
        [mesg setMessageSendState:STIMMessageSendState_Success];
...
- (void)receivePublicNumberMsg:(NSDictionary *)msgDic {
    ...
    NSString *extendInfo = [msgDic objectForKey:@"extendInfo"];
    if ([[IMDataManager stIMDB_SharedInstance] stIMDB_checkPublicNumberMsgById:msgId]) {
        return;
    }
    BOOL isSystemMsg = NO;
    ...
    if (msgType != PublicNumberMsgType_Action && msgType != PublicNumberMsgType_ClientCookie && msgType != PublicNumberMsgType_PostBackCookie && msgType != STIMMessageType_ConsultResult && msgType != MessageType_C2BGrabSingleFeedBack) {
        if (isSystemMsg) {
    ...
    if (dic == nil) {
        ...
        if (cardList.count <= 0) {
            ...
            [[IMDataManager stIMDB_SharedInstance] stIMDB_bulkInsertPublicNumbers:@[dic]];
        }
    }
    STIMMessageModel *c2bFeedBackMessage = [STIMMessageModel new];
    STIMMessageModel *message = [STIMMessageModel new];
    [message setMessageId:msgId];
    [message setFrom:publicNumberId];
    [message setRealJid:publicNumberId];
    [message setMessageDirection:STIMMessageDirection_Received];
    if (isSystemMsg) {
    ...
    [message setMessageSendState:STIMMessageSendState_Success];
    [message setMessageDate:msgDate];
    if (msgType != PublicNumberMsgType_Action && msgType != PublicNumberMsgType_ClientCookie && msgType != PublicNumberMsgType_PostBackCookie && msgType != STIMMessageType_ConsultResult) {
        [self saveMsg:message ByJid:publicNumberId];
    } else if (msgType == STIMMessageType_ConsultResult) {
        // 保存渠道信息
        if (message.extendInformation.length > 0) {
            [message setMessage:message.extendInformation];
        }
        NSDictionary *resultContent = [[STIMJSONSerializer sharedInstance] deserializeObject:extendInfo error:nil];
        ...
        [[IMDataManager stIMDB_SharedInstance] stIMDB_InsertOrUpdateUserInfos:@[userDic]];
        if (result) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (message.messageType == MessageType_C2BGrabSingleFeedBack) {
            ...
            STIMVerboseLog(@"抛出通知 STIMmanger receivePublicNumberMsg  kNotificationSessionListUpdate");
            ...
            if (![publicNumberId isEqualToString:[self getCurrentSessionUserId]] && message.messageDirection == STIMMessageDirection_Received) {
                if (isSystemMsg == NO) {
...
- (void)receiveCollectionMessage:(NSDictionary *)msgDic {
    ...
    STIMVerboseLog(@"receiveCollectionMessage msgDic : %@", msgDic);
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[STIMManager sharedInstance] checkMsgTimeWithJid:sid WithMsgDate:msgDate WithGroup:NO];
...
- (void)receiveCollectionOriginMessage:(NSDictionary *)msgDic {
    ...
    STIMVerboseLog(@"receiveCollectionOriginMessage : %@", msgDic);
    dispatch_async(self.receive_msg_queue, ^{
        NSString *msgId = [msgDic objectForKey:@"MsgId"];
        ...
        STIMVerboseLog(@"msgId : %@, originFrom : %@, originTo: %@, originType: %@", msgId, originFrom, originTo, originType);
        BOOL isExistCollectionMsg = [[IMDataManager stIMDB_SharedInstance] stIMDB_checkCollectionMsgById:msgId];
        if (!isExistCollectionMsg) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                STIMVerboseLog(@"抛出通知 STIMmanger receiveCollectionOriginMessage  kNotificationSessionListUpdate");
                [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationSessionListUpdate" object:nil];
            });
        }
    });
...
- (void)receiveConsultMessage:(NSDictionary *)msgDic{
    ...
    STIMVerboseLog(@"receiveConsultMessage : %@", msgDic);
    dispatch_async(self.receive_msg_queue, ^{
        ...
        NSString *msgId = [msgDic objectForKey:@"msgId"];
        NSString *msg = [msgDic objectForKey:@"msg"];
        long long msgDate = [[msgDic objectForKey:@"stamp"] stimDB_timeIntervalSince1970InMilliSecond];
        NSString *extendInfo = [msgDic objectForKey:@"extendInfo"];
        NSString *msgRaw = [msgDic objectForKey:@"msgRaw"];
        ...
        BOOL flag = [[IMDataManager stIMDB_SharedInstance] stIMDB_checkMsgId:msgId];
        if (flag) {
            return;
        }
        ...
        [mesg setMessageDirection:(isCarbon == YES) ? STIMMessageDirection_Sent : STIMMessageDirection_Received];
        [mesg setMessage:msg];
        [mesg setExtendInformation:extendInfo];
        [mesg setMessageDate:msgDate];
        [mesg setMsgRaw:msgRaw];
...
- (void)onReadState:(NSDictionary *)infoDic {
    STIMVerboseLog(@"onReadState : %@", infoDic);
    dispatch_async(self.receive_msgReadState_queue, ^{
        ...
        NSArray *readStateMsgList = [[STIMJSONSerializer sharedInstance] deserializeObject:infoStr error:nil];
        NSInteger remoteState = STIMMessageRemoteReadStateNotSent;
        if (readType == STIMMessageReadFlagClearAllUnRead) {
            remoteState = STIMMessageRemoteReadStateDidReaded;
            
            NSDictionary *readMarkTDic = [[STIMJSONSerializer sharedInstance] deserializeObject:infoStr error:nil];
            long long readMarkT = [[readMarkTDic objectForKey:@"T"] longLongValue];
            [[IMDataManager stIMDB_SharedInstance] stIMDB_updateAllMsgWithMsgRemoteState:remoteState ByMsgDirection:STIMMessageDirection_Received ByReadMarkT:readMarkT / 1000];
            [[IMDataManager stIMDB_SharedInstance] stIMDB_clearAtMessage];
            [self.hasAtAllDic removeAllObjects];
            dispatch_async(dispatch_get_main_queue(), ^{
                STIMVerboseLog(@"clearAllNoRead: 抛出通知 kMsgNotReadCountChange");
                [[NSNotificationCenter defaultCenter] postNotificationName:kMsgNotReadCountChange object:@{@"ForceRefresh":@(YES)}];
                STIMVerboseLog(@"抛出通知 clearAllNoRead: kAtALLChange");
                [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationSessionListUpdate object:nil];
            });
        } else if (readType == STIMMessageReadFlagGroupReaded) {
            //群已读
            long long maxMucReadMarkTime = ([[NSDate date] timeIntervalSince1970] - self.serverTimeDiff) * 1000;
            STIMVerboseLog(@"收到onReadState之后设置群阅读指针时间戳 : %lld", maxMucReadMarkTime);
            ...
            remoteState = STIMMessageRemoteReadStateDidReaded | STIMMessageRemoteReadStateGroupReaded;
            [self updateLocalGroupMessageRemoteState:remoteState withXmppId:jid ByReadList:readStateMsgList];
        } else {
            if (readType == STIMMessageReadFlagDidSend) {
                //已送到
                remoteState = STIMMessageRemoteReadStateDidSent;
            } else if (readType == STIMMessageReadFlagDidRead) {
                //已读
                remoteState = STIMMessageRemoteReadStateDidReaded;
            } else if (readType == STIMMessageReadFlagDidControl) {
                //已操作
                remoteState = STIMMessageRemoteReadStateDidOperated | STIMMessageRemoteReadStateDidReaded;
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationMessageControlStateUpdate" object:@{@"State":@(STIMMessageRemoteReadStateDidOperated), @"MsgIds":readStateMsgList?readStateMsgList:@[]}];
                });
            } else {
                //已发送
                remoteState = STIMMessageRemoteReadStateNotSent;
            }
...
- (void)onMessageStateUpdate:(NSDictionary *)msgDic {
    dispatch_async(self.receive_msgSendState_queue, ^{
        STIMVerboseLog(@"更新消息发送状态 : %@", msgDic);
        NSString *msgId = [msgDic objectForKey:@"messageId"];
        BOOL check = [[IMDataManager stIMDB_SharedInstance] stIMDB_checkMsgId:msgId];
        if (check == YES) {
            ...
            if (msgSuccess) {
                ...
                [[IMDataManager stIMDB_SharedInstance] stIMDB_updateMsgState:STIMMessageSendState_Success WithMsgId:msgId];
                
                [[IMDataManager stIMDB_SharedInstance] stIMDB_updateMsgDate:receivedTime WithMsgId:msgId];
                [[IMDataManager stIMDB_SharedInstance] stIMDB_updateMsgWithMsgRemoteState:STIMMessageRemoteReadStateNotSent ByMsgIdList:@[@{@"id":msgId}]];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageSendStateUpdate object:@{@"MsgSendState":@(STIMMessageSendState_Success), @"messageId":msgId}];
                });
            } else {
                //发送消息失败
                
                [[IMDataManager stIMDB_SharedInstance] stIMDB_updateMsgState:STIMMessageSendState_Faild WithMsgId:msgId];
                [[IMDataManager stIMDB_SharedInstance] stIMDB_updateMsgWithMsgRemoteState:STIMMessageRemoteReadStateNotSent ByMsgIdList:@[@{@"id":msgId}]];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageSendStateUpdate object:@{@"MsgSendState":@(STIMMessageSendState_Faild), @"messageId":msgId}];
                });
            }
        }
    });
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        switch (direction) {
            case MsgDirection_Send:
            {
                ...
                STIMVerboseLog(@"<Method: messageLogEvent, MsgDirection_Send :%@>", log);
            }
                break;
            case MsgDirection_Receive:
            {
                log = [NSString stringWithFormat:@"<=========== 接收了消息 ==========>\n%@",log];
                STIMVerboseLog(@"<Method: messageLogEvent, MsgDirection_Receive ：%@", log);
            }
                break;
            default:
                break;
...
+ (void)stimDB_privateCommonLog:(NSString *)log {
    STIMVerboseLog(@"stimDB_privateCommonLog ：%@", log);
}
...
- (void)serviceStreamEnd:(NSDictionary *)infoDic{
    STIMErrorLog(@"serviceStreamEnd : %@", infoDic);
    NSInteger errcode = [[infoDic objectForKey:@"ErrorCode"] integerValue];
    ...
    if (errcode >= 100 && errcode < 200) {
        if (errcode == 101) {
            ...
            STIMErrorLog(@"重新获取导航");
            [[STIMUserCacheManager sharedInstance] setUserObject:[STIMManager getLastUserName] forKey:@"currentLoginName"];
            BOOL getNavSuccess = [[STIMNavConfigManager sharedInstance] qimNav_updateNavigationConfigWithCheck:YES];
            if (getNavSuccess == NO) {
                STIMErrorLog(@"获取导航失败，请稍后再试");
                [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(checkNetworkStatus) object:nil];
...
- (void)pbChatRoomAddMember:(NSDictionary *)infoDic{
    dispatch_async(self.update_group_member_queue, ^{
        ...
        if ([[IMDataManager stIMDB_SharedInstance] stIMDB_checkGroup:groupId] == NO) {
        ...
        [memberInfoDic setSTIMSafeObject:affiliation forKey:@"affiliation"];
        if (memberJid) {
            [[IMDataManager stIMDB_SharedInstance] stIMDB_insertGroupMember:memberInfoDic WithGroupId:groupId];
        }
    });
}
...
- (void)pbChatRoomDestory:(NSDictionary *)infoDic{
    dispatch_async(self.receive_msg_queue, ^{
        ...
        [[IMDataManager stIMDB_SharedInstance] stIMDB_deleteMessageWithXmppId:groupId];
...
- (void)pbChatRoomCardUpdate:(NSDictionary *)infoDic{
    ...
    [groupInfoDic setSTIMSafeObject:groupId forKey:@"MN"];
    [groupInfoDic setSTIMSafeObject:nickName forKey:@"SN"];
    [groupInfoDic setSTIMSafeObject:desc forKey:@"MD"];
    [groupInfoDic setSTIMSafeObject:title forKey:@"MT"];
    [groupInfoDic setSTIMSafeObject:pic forKey:@"MP"];
    [groupInfoDic setSTIMSafeObject:version forKey:@"VS"];
    if (groupInfoDic) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            [[IMDataManager stIMDB_SharedInstance] stIMDB_bulkUpdateGroupCards:@[groupInfoDic]];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)friendValidation:(NSDictionary *)validationDic {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            STIMVerboseLog(@"抛出通知 STIMmanger friendValidation1  kNotificationSessionListUpdate");
            [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationSessionListUpdate object:nil];
        });
    });
}
...
- (void)friendPresence:(NSDictionary *)presenceDic {
    ...
    if ([result isEqualToString:@"success"]) {
        ...
        [[IMDataManager stIMDB_SharedInstance] stIMDB_insertFriendWithUserId:[infoDic objectForKey:@"UserId"]
...
- (void)userMucListUpdate:(NSDictionary *)mucListDic {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        if ((requestFlag == nil || requestFlag.boolValue) && mucListStr) {
            ...
            STIMVerboseLog(@"========= 开始接受群列表数据，同步本地数据 ======= : %@", newGroupIds);
...
- (void)configForRemoteKeyAndSystemTime:(NSDictionary *)configDic {
    ...
    { //U
        ...
        [tcookieProperties setSTIMSafeObject:@"_u" forKey:NSHTTPCookieName];
        [tcookieProperties setSTIMSafeObject:[STIMManager getLastUserName] forKey:NSHTTPCookieValue];
    ...
    { //K
        ...
        NSMutableDictionary *tcookieProperties = [NSMutableDictionary dictionary];
        [tcookieProperties setSTIMSafeObject:@"_k" forKey:NSHTTPCookieName];
        [tcookieProperties setSTIMSafeObject:self.remoteKey forKey:NSHTTPCookieValue];
        [tcookieProperties setSTIMSafeObject:@".qunar.com" forKey:NSHTTPCookieDomain];
        [tcookieProperties setSTIMSafeObject:@"/" forKey:NSHTTPCookiePath];
        [tcookieProperties setSTIMSafeObject:@"0" forKey:NSHTTPCookieVersion];
...
- (void)loginFaild:(NSDictionary *)errDic {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(checkNetworkStatus) object:nil];
    dispatch_async(dispatch_get_main_queue(), ^{
        STIMErrorLog(@"LoginFaild: %@", errDic);
