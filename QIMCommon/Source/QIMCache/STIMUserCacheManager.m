...
@interface STIMUserCacheManager () <NSCacheDelegate>
...
@implementation STIMUserCacheManager {
...
+ (instancetype)sharedInstance {
    static STIMUserCacheManager *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[STIMUserCacheManager alloc] init];
    });
    return instance;
}
...
- (YYCache *)yyCacheManager {
    if (!_yyCacheManager) {
        ...
        _yyCacheManager = [YYCache cacheWithName:[self getSTIMCacheName]];
...
- (BOOL)isNeedSaveUserDefaultKey:(NSString *)aKey {
    return [self.needSaveUserDefaultKeys containsObject:aKey] || [[self getSTIMCacheName] isEqualToString:@"APP"];
}
