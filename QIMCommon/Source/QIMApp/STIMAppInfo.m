...
@interface STIMAppInfo ()
// 内存缓存，初始化时需要清零
@property (nonatomic, strong) NSString *macAddress;                // 网卡mac地址

@end

static STIMAppInfo *__globalAppInfo = nil;

@implementation STIMAppInfo

+ (id)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __globalAppInfo = [[STIMAppInfo alloc] init];
...
- (NSString *)getPushToken {
    if (!_pushToken) {
        _pushToken = [[STIMUserCacheManager sharedInstance] userObjectForKey:@"AppPushToken"];
    }
    return _pushToken;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"pushToken"]) {
        STIMWarnLog(@"PushToken 改变: %@", change);
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_TARGET_QUEUE_DEFAULT, 0), ^{
            if (_pushToken) {
                [[STIMUserCacheManager sharedInstance] setUserObject:_pushToken forKey:@"AppPushToken"];
            }
            [[STIMManager sharedInstance] sendPushTokenWithMyToken:[[STIMAppInfo sharedInstance] pushToken] WithDeleteFlag:deleteFlag];
        });
    } else {
...
- (NSString *)appAID
{
    return [STIMUUIDTools deviceUUID];
}
