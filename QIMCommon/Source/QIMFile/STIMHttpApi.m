...
@interface STIMHttpApi(PrivateCommonMethod)
...
+ (NSString *)checkFileKeyForFile:(NSString *)fileKey WithFileLength:(long long)fileLength WithPathExtension:(NSString *)extension{
    ...
    NSString *destUrl = [NSString stringWithFormat:@"%@/%@?key=%@&size=%lld&name=%@&platform=iphone&u=%@&k=%@&version=%@",
                         [STIMNavConfigManager sharedInstance].innerFileHttpHost, method, fileKey, (long long)ceil(fileLength / 1024.0 / 1024.0), [NSString stringWithFormat:@"%@.%@",fileKey,extension],
...
+(NSString*)getFileMD5WithPath:(NSString*)path{
    return (__bridge_transfer NSString *)STIMFileMD5HashCreateWithPath((__bridge CFStringRef)path, FileHashDefaultChunkSizeForReadingData);
}
...
+ (NSDictionary *)checkUserToken:(NSString *)token{
    ...
    NSString *destUrl = [[STIMNavConfigManager sharedInstance] checkSmsUrl];
    NSURL *requestUrl = [[NSURL alloc] initWithString:destUrl];
...
+ (NSString *)updateLoadMomentImage:(NSData *)fileData WithMsgId:(NSString *)key WithMsgType:(int)type WithPathExtension:(NSString *)extension{
    ...
    NSString *destUrl = [NSString stringWithFormat:@"%@/%@?name=%@&p=ios&u=%@&k=%@&v=%@&key=%@&size=%lld",
                         ...
                         [[STIMManager getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
                         [[STIMManager sharedInstance] myRemotelogginKey],
                         [[STIMAppInfo sharedInstance] AppBuildVersion],key,size];
    ...
    if ([formRequest responseStatusCode] == 200) {
        NSDictionary *result = [[STIMJSONSerializer sharedInstance] deserializeObject:formRequest.responseData error:nil];
...
+ (NSString *)updateLoadImage:(NSData *)fileData WithMsgId:(NSString *)key WithMsgType:(int)type WithPathExtension:(NSString *)extension{
    ...
    [formRequest setPostFormat:ASIMultipartFormDataPostFormat];
    [formRequest addData:fileData withFileName:fileName andContentType:nil forKey:@"file"];
    STIMHttpAPIBlock *progressBlock = [[STIMHttpAPIBlock alloc] init];
...
+ (NSString *) updateLoadMomentFile:(NSData *)fileData WithMsgId:(NSString *)key WithMsgType:(int)type WithPathExtension:(NSString *)extension{
    ...
    if (httpUrl == nil) {
        return [STIMHttpApi updateLoadMomentImage:fileData WithMsgId:fileKey WithMsgType:type WithPathExtension:extension];
    }
    return httpUrl;
}
...
+ (NSString *) updateLoadFile:(NSData *)fileData WithMsgId:(NSString *)key WithMsgType:(int)type WithPathExtension:(NSString *)extension{
    ...
    NSString *httpUrl = [self checkFileKey:fileKey WithFileLength:fileData.length WithPathExtension:extension];
    if (httpUrl == nil) {
        return [STIMHttpApi updateLoadImage:fileData
                                WithMsgId:fileKey
                              WithMsgType:type
...
+ (NSDictionary *)checkVideo:(NSString *)fileMd5 {
    NSString *destUrl = [NSString stringWithFormat:@"%@/video/check", [[STIMNavConfigManager sharedInstance] newerHttpUrl]];
    ...
    NSMutableData *postData = [NSMutableData dataWithData:[[STIMJSONSerializer sharedInstance] serializeObject:bodyDic error:nil]];
...
+ (void)uploadVideo:(NSData *)fileData withCallBack:(STIMKitUploadVideoRequestSuccessedBlock)callback {
    NSString *fileMd5 = [[fileData mutableCopy] stimDB_md5String];
    if (fileMd5.length > 0) {
        ...
        if (checkResultDic) {
            ...
            [[STIMManager sharedInstance] uploadFileRequest:destUrl withFileData:fileData withProgressBlock:nil withSuccessCallBack:^(NSData *responseData) {
...
+ (void)uploadVideoPath:(NSString *)filePath withCallBack:(STIMKitUploadVideoRequestSuccessedBlock)callback {
    ...
    if (fileMd5.length > 0) {
        ...
        if (checkResultDic) {
            ...
            NSString *destUrl = [NSString stringWithFormat:@"%@/video/upload", [[STIMNavConfigManager sharedInstance] newerHttpUrl]];
            [[STIMManager sharedInstance] uploadFileRequest:destUrl withFilePath:filePath withProgressBlock:nil withSuccessCallBack:^(NSData *responseData) {
                NSDictionary *resultDic = [[STIMJSONSerializer sharedInstance] deserializeObject:responseData error:nil];
                NSLog(@"resultDic : %@", resultDic);
                BOOL ret = [[resultDic objectForKey:@"ret"] boolValue];
                if (ret) {
...
+ (NSString *)updateLoadVoiceFile:(NSData *)voiceFileData WithFilePath:(NSString *)filePath {
    ...
    if (httpUrl == nil) {
        return [STIMHttpApi updateLoadVoiceFile_New:voiceFileData WithFilePath:filePath WithFileKey:fileKey];
    }
    return httpUrl;
}
...
@implementation STIMHttpApi(CommonMethod)
...
+ (NSDictionary *)deserializeAsDictionary:(NSData *)data
{
    ...
    dictionary = [[STIMJSONSerializer sharedInstance] deserializeObject:jsonData error:nil];
...
@implementation STIMHttpApi(PrivateCommonMethod)
