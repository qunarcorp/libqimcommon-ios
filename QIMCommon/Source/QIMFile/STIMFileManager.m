...
@interface STIMFileManagerSizeManager :NSObject

@end

@implementation STIMFileManagerSizeManager
...
@interface STIMFileRequest : NSObject <ASIHTTPRequestDelegate,ASIProgressDelegate>
@property (nonatomic, assign) ASIHTTPRequest *fileRequest;
@property (nonatomic, assign) FileRequestType fileReuqestType;
@property (nonatomic, assign) STIMFileCacheType fileCacheType;
...
@implementation STIMFileRequest{
...
- (void)dealloc{
    STIMErrorLog(@"STIMFileRequest Dealloc : %p", self);
...
- (void)cancelToJid:(NSString *)jid{
    if (jid) {
        ...
        if (_sendToJid.count <= 0) {
            [[STIMFileManager sharedInstance] removeFileId:self.fileId];
        }
    }
}
...
- (void)requestFinished:(ASIHTTPRequest *)request{
    ...
    if (self.fileReuqestType == FileRequest_Upload) {
        ...
        STIMVerboseLog(@"上传真正的文件结果 : %@ ForMessage: %@", [request responseString], self.message);
        if ([request responseStatusCode] == 200) {
            ...
            NSDictionary *result = [[STIMJSONSerializer sharedInstance] deserializeObject:data error:nil];
                ...
                if (self.newVideoInterface == YES) {
                    ...
                    if ([resultUrl isEqual:[NSNull null]] == NO && resultUrl.length > 0) {
                        ...
                        /*
                         ...
                         resultUrl = [url path];
                         NSString *innerFileUrlPath = [[NSURL URLWithString:[[STIMNavConfigManager sharedInstance] innerFileHttpHost]] path];
                         ...
                         NSDictionary *queryDic = [[url query] stimDB_dictionaryFromQueryComponents];
            ...
            //更新消息时间
            long long msgDate = ([[NSDate date] timeIntervalSince1970] - [[STIMManager sharedInstance] getServerTimeDiff])*1000;
...
//            QTEncryptChatState encryptState = [[QTEncryptChat sharedInstance] getEncryptChatStateWithUserId:self.message.to];
            if (self.message.messageType == STIMMessageType_SmallVideo) {
                if (self.newVideoInterface == YES) {
                    ...
                    NSString *fileSizeStr = [STIMFileManagerSizeManager stimDB_CapacityTransformStrWithSize:videoSize];
                    ...
                    [newVideoDic setSTIMSafeObject:@(Duration) forKey:@"Duration"];
                    [newVideoDic setSTIMSafeObject:videoName forKey:@"FileName"];
                    [newVideoDic setSTIMSafeObject:fileSizeStr forKey:@"FileSize"];
                    [newVideoDic setSTIMSafeObject:videoUrl forKey:@"FileUrl"];
                    [newVideoDic setSTIMSafeObject:height forKey:@"Height"];
                    [newVideoDic setSTIMSafeObject:ThumbName forKey:@"ThumbName"];
                    [newVideoDic setSTIMSafeObject:firstThumbUrl forKey:@"ThumbUrl"];
                    [newVideoDic setSTIMSafeObject:width forKey:@"Width"];
                    if (self.transVideo == YES) {
                        [newVideoDic setSTIMSafeObject:@(YES) forKey:@"newVideo"];
                    } else {
                        [newVideoDic setSTIMSafeObject:@(NO) forKey:@"newVideo"];
                    }
                    
                    NSString *msg = [[STIMJSONSerializer sharedInstance] serializeObject:newVideoDic];
                    ...
                    self.message.messageType = STIMMessageType_SmallVideo;
                    if (self.message.chatType == ChatType_PublicNumber) {
                        [[STIMManager sharedInstance] sendMessage:msg ToPublicNumberId:self.toJid WithMsgId:self.message.messageId WithMsgType:self.message.messageType];
                    } else if (self.message.chatType == ChatType_Consult || self.message.chatType == ChatType_ConsultServer) {
                        [[STIMManager sharedInstance] sendConsultMessageId:self.message.messageId WithMessage:self.message.message WithInfo:self.message.extendInformation toJid:self.message.to realToJid:self.message.realJid WithChatType:self.message.chatType WithMsgType:self.message.messageType];
                    } else {
                        [[STIMManager sharedInstance] sendMessage:self.message ToUserId:self.message.to];
                    }
                } else {
                    if ([[infoDic objectForKey:@"msgType"] integerValue] == STIMMessageType_SmallVideo && self.message.extendInformation.length > 0) {
                        self.message.message = [[[STIMJSONSerializer sharedInstance] deserializeObject:self.message.extendInformation error:nil] objectForKey:@"message"];
                    }
                    ...
                    if (dic) {
                        ...
                        NSString *msg = [[STIMJSONSerializer sharedInstance] serializeObject:infoDic];
                        
                        if (![httpUrl stimDB_hasPrefixHttpHeader]){
                            httpUrl = [[STIMNavConfigManager sharedInstance].innerFileHttpHost stringByAppendingFormat:@"/%@", httpUrl];
                        }
...
//                            NSString *encryptContent = [[QTEncryptChat sharedInstance] encryptMessageWithMsgType:STIMMessageType_SmallVideo WithOriginBody:messageContent WithOriginExtendInfo:msg WithUserId:self.message.to];
//                            self.message.message = @"加密视频消息iOS";
//                            self.message.extendInformation = encryptContent;
//                            self.message.messageType = STIMMessageType_Encrypt;
//                        } else {
...
//                        }
                        if (self.message.chatType == ChatType_PublicNumber) {
                            ...
                            [[STIMManager sharedInstance] sendMessage:self.message ToUserId:self.toJid];
                        }
                    }
                }
            ...
            } else if (self.message.messageType == STIMMessageType_LocalShare){
                NSDictionary *dic = [[STIMJSONSerializer sharedInstance] deserializeObject:self.message.originalExtendedInfo error:nil];
                NSMutableDictionary * mulDic = [NSMutableDictionary dictionaryWithDictionary:dic];
                [mulDic setSTIMSafeObject:httpUrl forKey:@"fileUrl"];
                NSString *msgExtendInfo = [[STIMJSONSerializer sharedInstance] serializeObject:mulDic];
                //STIMSDKTODO
//                if (encryptState == QTEncryptChatStateEncrypting) {
//                    NSString *encryptContent = [[QTEncryptChat sharedInstance] encryptMessageWithMsgType:STIMMessageType_LocalShare WithOriginBody:[[self message] originalMessage] WithOriginExtendInfo:msgExtendInfo WithUserId:self.message.to];
...
//                }
            ...
            }else if (self.message.messageType == STIMMessageType_CommonTrdInfo) {
                NSMutableDictionary * mulDic = [NSMutableDictionary dictionaryWithDictionary:infoDic];
                if (![httpUrl stimDB_hasPrefixHttpHeader]) {
                    httpUrl = [NSString stringWithFormat:@"%@/%@", [STIMNavConfigManager sharedInstance].innerFileHttpHost, httpUrl];
                }
                ...
                NSString *shareurl = [NSString stringWithFormat:@"%@?jdata=%@", [[STIMNavConfigManager sharedInstance] shareUrl], [jDataStr stimDB_URLEncodedString]];
                [mulDic setSTIMSafeObject:shareurl forKey:@"linkurl"];
                NSString *msgExtendInfoStr = [[STIMJSONSerializer sharedInstance] serializeObject:mulDic];
                //STIMSDKTODO
//                if (encryptState == QTEncryptChatStateEncrypting) {
//                    NSString *encryptContent = [[QTEncryptChat sharedInstance] encryptMessageWithMsgType:STIMMessageType_CommonTrdInfo WithOriginBody:@"您收到了一个消息记录文件文件，请升级客户端查看。" WithOriginExtendInfo:msgExtendInfoStr WithUserId:self.message.to];
...
//                [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationFileDidUpload object:self.message];
            ...
            }else if (self.message.messageType == STIMMessageType_File) {
                NSMutableDictionary * mulDic = [NSMutableDictionary dictionaryWithDictionary:infoDic];
                [mulDic setSTIMSafeObject:httpUrl forKey:@"HttpUrl"];
                NSString *msgContent = [[STIMJSONSerializer sharedInstance] serializeObject:mulDic];
                //STIMSDKTODO
//                if (encryptState == QTEncryptChatStateEncrypting) {
//                    NSString *encryptContent = [[QTEncryptChat sharedInstance] encryptMessageWithMsgType:STIMMessageType_File WithOriginBody:msgContent WithOriginExtendInfo:msgContent WithUserId:self.message.to];
//                    self.message.message = @"加密消息记录消息iOS";
//                    self.message.extendInformation = encryptContent;
//                    self.message.messageType = STIMMessageType_Encrypt;
//                } else {
...
//                }
                ...
                NSString *extendInfo = [[STIMJSONSerializer sharedInstance] serializeObject:extendInfoDic];
                if ([httpUrl length] > 0) {
                    self.message.message = string;
                    self.message.extendInformation = extendInfo;
                    NSString *tempHttpUrl = httpUrl;
                    if (![tempHttpUrl stimDB_hasPrefixHttpHeader]){
                        tempHttpUrl = [[STIMNavConfigManager sharedInstance].innerFileHttpHost stringByAppendingFormat:@"/%@", tempHttpUrl];
                    }
                        //STIMSDKTODO
//                        if (encryptState == QTEncryptChatStateEncrypting) {
...
//                    self.message.messageSendState = MessageState_Success;
                    if (self.message.chatType == ChatType_PublicNumber) {
                        [[STIMManager sharedInstance] sendMessage:self.message.message ToPublicNumberId:self.toJid WithMsgId:self.message.messageId WithMsgType:self.message.messageType];
                    } else if (self.message.chatType == ChatType_Consult) {
                        NSDictionary *infoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:self.message.ochatJson error:nil];
                        ...
                        NSString * cctntJsonStr = [[STIMJSONSerializer sharedInstance] serializeObject:cctntDic];
        ...
        STIMFileCacheType fileCacheType = self.fileCacheType;
        NSString *url = request.url.absoluteString;
        if (!error && [request responseStatusCode] == 200) {
            NSData *responseData = [request responseData].length > 0 ? [request responseData] : _receiveData ;
            if ([responseData length] > 0) {
                
                NSString * fileName = [[STIMFileManager sharedInstance] saveFileData:responseData url:self.message.messageId forCacheType:fileCacheType];
...
//            [[STIMDataController getInstance] saveResourceWithFileName:self.fileUrl data:[_fileHandle readDataToEndOfFile]];
    ...
    if (fileId) {
        [[STIMFileManager sharedInstance] removeFileId:fileId];
    }
}
...
@interface STIMFileManager () <ASIProgressDelegate>

@property (nonatomic, strong) NSMutableDictionary *file_dic;

@end

@implementation STIMFileManager {
...
+ (STIMFileManager *)sharedInstance {
    ...
    static STIMFileManager *monitor = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        monitor = [[STIMFileManager alloc] init];
    });
    return monitor;
}
...
+ (NSString *) documentsofPath:(STIMFileCacheType) type {
    ...
    if (type == STIMFileCacheTypeColoction) {
...
- (void)checkUploadFile{
    ...
    if (nextFileId) {
        STIMFileRequest *nextRequest = [self.file_dic objectForKey:nextFileId];
...
- (void)removeFileId:(NSString *)fileId{
    ...
    STIMFileRequest *request = [self.file_dic objectForKey:fileId];
...
- (NSString *)uploadFileForData:(NSData *)fileData forMessage:(STIMMessageModel *)message withJid:(NSString *)jid isFile:(BOOL)flag {
    ...
    NSDictionary *msgInfoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:message.message error:nil];
    ...
    BOOL isVideo = (message.messageType == STIMMessageType_SmallVideo);
    BOOL videoConfigUseAble = [[[STIMUserCacheManager sharedInstance] userObjectForKey:@"VideoConfigUseAble"] boolValue];
    ...
    if (isVideo && videoConfigUseAble) {
        ...
        //限制视频时长
        NSInteger videoTimeLen = [[[STIMUserCacheManager sharedInstance] userObjectForKey:@"videoTimeLen"] integerValue] / 1000;
        NSDictionary *videoExt = [[STIMJSONSerializer sharedInstance] deserializeObject:message.message error:nil];
        NSInteger videoDuration = [[videoExt objectForKey:@"Duration"] integerValue];

        
        NSString *destUrl = [NSString stringWithFormat:@"%@/video/upload", [[STIMNavConfigManager sharedInstance] newerHttpUrl]];
        ...
        STIMFileRequest *fileRequest = [[STIMFileRequest alloc] init];
    ...
    //存小图
    [self saveImageData:fileData withFileName:fileName width:size.width height:size.height forCacheType:STIMFileCacheTypeColoction];
    [[STIMHttpRequestMonitor sharedInstance] syncRunBlock:^{
        //存原图
        [self saveFileData:fileData withFileName:fileName forCacheType:STIMFileCacheTypeColoction];
        ...
        NSString *destUrl = [NSString stringWithFormat:@"%@/%@?key=%@&size=%lld&name=%@&p=iphone&u=%@&k=%@&version=%@",
                             [STIMNavConfigManager sharedInstance].innerFileHttpHost, method, fileKey, fileLength, fileName,
                             [[STIMManager getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
                             [[STIMManager sharedInstance] myRemotelogginKey],
            ...
            if (!ret) {
                ...
                if ([resultUrl isEqual:[NSNull null]] == NO && resultUrl) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        NSData *data = [[STIMJSONSerializer sharedInstance] serializeObject:resultDic error:nil];
...
- (void)uploadFileForData:(NSData *)fileData fileKey:(NSString *)fileKey fileExt:(NSString *)fileExt forMessage:(STIMMessageModel *)message withJid:(NSString *)jid isFile:(BOOL)flag {
    ...
    NSString *destUrl = [NSString stringWithFormat:@"%@/%@?name=%@&p=iphone&u=%@&k=%@&v=%@&key=%@&size=%lld",
                         [STIMNavConfigManager sharedInstance].innerFileHttpHost, method, fileName,
                         [[STIMManager getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
                         [[STIMManager sharedInstance] myRemotelogginKey],
                         [[STIMAppInfo sharedInstance] AppBuildVersion],fileKey,size];
    STIMVerboseLog(@"上传真正的文件 : %@ ForMessage: %@", destUrl, message);
...
- (void)uploadAsynchronousForFileData:(NSData *)fileData fileKey:(NSString *)fileKey fileName:(NSString *)fileName progressDelegate:(id)delegate isFile:(BOOL)flag completeblock:(void(^)(NSString * resultUrl))completeblock progressBlock:(void(^)(CGFloat progress))progressBlock{
    ...
    [request setCompletionBlock:^{
        if ([weakRequest responseStatusCode] == 200) {
            NSDictionary *result = [[STIMJSONSerializer sharedInstance] deserializeObject:weakRequest.responseData error:nil];
            BOOL ret = [[result objectForKey:@"ret"] boolValue];
            if (ret) {
                NSString *resultUrl = [result objectForKey:@"data"];
                if (completeblock) {
...
- (void)uploadFileForData:(NSData *)fileData forCacheType:(STIMFileCacheType)type isFile:(BOOL)flag fileExt:(NSString *)fileExt completionBlock:(STIMFileManagerUploadCompletionBlock)completionBlock {
    ...
    dispatch_async(_file_queue, ^{
        ...
        if (resultUrl) {
            [[STIMFileManager sharedInstance] saveFileData:fileData url:resultUrl forCacheType:STIMFileCacheTypeColoction];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)uploadFileForData:(NSData *)fileData
   ...
   uploadProgressDelegate:(id)delegate
          completionBlock:(STIMFileManagerUploadCompletionBlock)completionBlock
...
-(void)downloadCollectionEmoji:(NSString *)url
    ...
    [[STIMHttpRequestMonitor sharedInstance] runblock:^{
        
        NSString *filePath = [[STIMFileManager documentsofPath:type] stringByAppendingPathComponent:md5];
        if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
            if (!complation) {
                dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [[STIMHttpRequestMonitor sharedInstance] runblock:^{
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
           ...
           STIMMessageModel *msg = [STIMMessageModel new];
            ...
            STIMFileRequest *fileRequest = [weakSelf.file_dic objectForKey:msg.messageId];
            if (!fileRequest) {
                fileRequest = [[STIMFileRequest alloc] init];
...
- (NSString *)getNewMd5ForMd5:(NSString *)oldMd5 withWidth:(float)width height:(float)height {
    ...
    if (intWidth && intHeight) {
        newMd5 = [[NSString stringWithFormat:@"%@_w%@_h%@", oldMd5, @(intWidth), @(intHeight)] stimDB_getMD5];
    }
    return newMd5;
}
...
- (NSData *) getFileDataForFileName:(NSString *)fileName forCacheType:(STIMFileCacheType)type {
    if (![fileName stimDB_isStringSafe])
    {
        return nil;
    }
...
- (NSData *) getFileDataFromUrl:(NSString *)url width:(float)width height:(float)height forCacheType:(STIMFileCacheType)type needUpdate:(BOOL)needUpdate {
 ...
 *  @param type     文件来源类型
 */
- (NSData *)getResourceWithFileName:(NSString *)fileName forCacheType:(STIMFileCacheType)type
{
    NSData *data = nil;
...
- (NSString *)fileExistLocalPathForUrl:(NSString *)url width:(float)width height:(float)height forCacheType:(STIMFileCacheType)type {
    NSString * fileName = [self getFileNameFromUrl:url width:width height:height];
    NSString *filePath = [[STIMFileManager documentsofPath:type] stringByAppendingPathComponent:fileName];
    return filePath;
}
...
- (NSString *) saveImageData:(NSData *)data withFileName:(NSString *)fileName width:(float)width height:(float)height forCacheType:(STIMFileCacheType)type{
 ...
 *  @param type     文件来源类型
 */
- (NSString *) saveFileData:(NSData *)data withFileName:(NSString *)fileName forCacheType:(STIMFileCacheType)type update:(BOOL) update{
    if (fileName == nil) {
        fileName = [self getMD5FromFileData:data];
    }
    
    NSString *cachePath = [STIMFileManager documentsofPath:type];
...
- (NSString *)getMD5FromFileData:(NSData *)fileData{
    return (__bridge_transfer NSString *)STIMFileMD5HashCreateWithData([fileData bytes], fileData.length);
}
...
- (void )getPermUrlWithTempUrl:(NSString *)tempUrl PermHttpUrl:(void(^)(NSString *))callBackPermUrl{
    ...
    [[STIMHttpRequestMonitor sharedInstance] syncRunBlock:^{
        ...
        NSString *PerString = [NSString stringWithFormat:@"%@/file/v2/stp?url=%@",
                               [[STIMNavConfigManager sharedInstance] innerFileHttpHost],
                               compnentString];
        ...
        if (!error) {
            NSError *error = nil;
            NSDictionary *dict = [[STIMJSONSerializer sharedInstance] deserializeObject:request.responseData error:&error];
            httpPermUrl = [NSString stringWithFormat:@"%@", dict[@"data"]];
        }
        
    } url:tempUrl];
    NSString *md5 = [[STIMFileManager sharedInstance] getFileNameFromUrl:httpPermUrl];
