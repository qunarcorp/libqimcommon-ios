//
//  IMDataManager+STIMDBClientConfig.m
//  STIMCommon
//
...
@implementation IMDataManager (STIMDBClientConfig)
...
- (NSMutableDictionary *)stIMDB_getConfigDicWithConfigKey:(NSString *)configKey WithDeleteFlag:(BOOL)deleteFlag {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        ...
        while ([reader read]) {
            ...
            [dic setSTIMSafeObject:configSubKey forKey:@"ConfigSubKey"];
            [dic setSTIMSafeObject:configValue forKey:@"ConfigValue"];
            
            [result setSTIMSafeObject:dic forKey:configSubKey];
        }
    }];
    STIMVerboseLog(@"");
    return result;
...
- (NSArray *)stIMDB_getConfigInfoArrayWithConfigKey:(NSString *)configKey WithDeleteFlag:(BOOL)deleteFlag {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        ...
        while ([reader read]) {
            ...
            [dic setSTIMSafeObject:configSubKey forKey:@"ConfigSubKey"];
            [dic setSTIMSafeObject:configValue forKey:@"ConfigValue"];
            [dic setSTIMSafeObject:@(deleteFlag) forKey:@"DeleteFlag"];
...
//            if (![headUrl stimDB_hasPrefixHttpHeader] && [headUrl hasPrefix:@"file/v"]) {
//                headUrl = [NSString stringWithFormat:@"%@/%@", [[STIMKit sharedInstance] qimNav_InnerFileHttpHost], headUrl];
//            }
