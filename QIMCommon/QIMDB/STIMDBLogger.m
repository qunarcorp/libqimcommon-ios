...
@interface STIMDBLogger ()

- (void)validateLogDirectory;

@end

@interface STIMDBLogEntry : NSObject {
@public
    NSNumber * context;
    NSNumber * level;
    NSString * message;
...
- (void)validateLogDirectory
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:logDirectory isDirectory:&isDirectory])
    {
        if (!isDirectory)
        {
            STIMVerboseLog(@"%@: %@ - logDirectory(%@) is a file!", [self class], THIS_METHOD, logDirectory);
    ...
    else
    {
        ...
        if (!result)
        {
            STIMVerboseLog(@"%@: %@ - Unable to create logDirectory(%@) due to error: %@",
...
- (BOOL)db_log:(DDLogMessage *)logMessage
{
    ...
    STIMDBLogEntry *logEntry = [[STIMDBLogEntry alloc] initWithLogMessage:logMessage];
...
- (void)db_save
{
    ...
    /* Mark DBUpadte
    [dbOperator syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        NSString *cmd = @"INSERT INTO logs (context, level, message, timestamp) VALUES (?, ?, ?, ?)";
        NSMutableArray *params = [[NSMutableArray alloc] init];
        for (STIMDBLogEntry *logEntry in self->pendingLogEntries) {
        ...
        if (!result) {
            STIMVerboseLog(@"Error inserting log entries");
        }
    }];
    */
    ...
    for (STIMDBLogEntry *logEntry in pendingLogEntries)
    {
    ...
    if (saveOnlyTransaction)
    {
        ...
        {
            STIMVerboseLog(@"%@: Error inserting log entries: code(%d): %@",
