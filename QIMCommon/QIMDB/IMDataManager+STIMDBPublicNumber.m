//
//  IMDataManager+STIMDBPublicNumber.m
//  STIMCommon
//
...
@implementation IMDataManager (STIMDBPublicNumber)
...
- (void)stIMDB_checkPublicNumbers:(NSArray *)publicNumberIds {
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        ...
        STIMVerboseLog(@"检查公众号列表%ld条数据, 耗时 = %f s", publicNumberIds.count, end - start); //s
    }];
}
