//
//  IMDataManager+STIMDBCollectionMessage.m
//  STIMCommon
//
...
@implementation IMDataManager (STIMDBCollectionMessage)
...
- (NSArray *)stIMDB_getCollectionSessionListWithBindId:(NSString *)bindId {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        ...
        while ([reader read]) {
            ...
            [IMDataManager safeSaveForDic:sessionDic setObject:@(STIMMessageSendState_Success) forKey:@"MsgState"];
            [IMDataManager safeSaveForDic:sessionDic setObject:@(STIMMessageDirection_Received) forKey:@"MsgDirection"];
...
- (void)stIMDB_updateCollectionMsgNotReadStateForBindId:(NSString *)bindId originUserId:(NSString *)originUserId WithReadState:(NSInteger)readState{
    NSArray *msgList = [self stIMDB_getCollectionMsgListWithUserId:bindId originUserId:originUserId];
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
