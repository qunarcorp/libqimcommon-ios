...
@implementation IMDataManager (STIMDBGroup)
...
//    [[STIMWatchDog sharedInstance] start];
    ...
    STIMVerboseLog(@"");
//    STIMVerboseLog(@"数据库取群名片耗时 : %lf", [[STIMWatchDog sharedInstance] escapedTime]);
    return groupCardDic;
}
...
- (void)stIMDB_bulkUpdateIncrementGroupCards:(NSArray *)array{
    ...
    [[IMDataManager stIMDB_SharedInstance] stIMDB_UpdateUserCacheDataWithKey:@"kGetIncrementMucGroupCardVersion" withType:12 withValue:@"获取群增量名片" withValueInt:maxUTTime];
    STIMVerboseLog(@"");
}

- (NSInteger)stIMDB_getGroupListMaxUTLastUpdateTime {
    NSInteger lastMaxUTTime = [[IMDataManager stIMDB_SharedInstance] stIMDB_getUserCacheDataWithKey:@"kGetIncrementMucGroupCardVersion" withType:12];
