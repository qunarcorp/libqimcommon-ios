...
@implementation IMDataManager (STIMDBUser)
...
- (void)stIMDB_bulkInsertOrgansUserInfos:(NSArray *)userInfos {
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        ...
        for (NSDictionary *infoDic in userInfos) {
            ...
            NSString *xmppId = [NSString stringWithFormat:@"%@@%@",userId, [[IMDataManager stIMDB_SharedInstance] getDBOwnerDomain]];
...
- (void)stIMDB_bulkInsertUserInfosNotSaveDescInfo:(NSArray *)userInfos{
    ...
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        ...
        STIMVerboseLog(@"更新组织架构%ld条数据 耗时 = %f s", userInfos.count, end - start); //s
    }];
}
...
- (NSDictionary *)stIMDB_getUserMarkNameDic {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase * _Nonnull db) {
        ...
        while ([reader read]) {
            ...
            [resultDic setSTIMSafeObject:userMarkName forKey:xmppId];
        }
    }];
    return resultDic;
}
...
- (void)stIMDB_updateUser:(NSString *)userId WithMood:(NSString *)mood WithHeaderSrc:(NSString *)headerSrc WithVersion:(NSString *)version{
    ...
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"更新用户信息 耗时 = %f s userId : %@, headerSrc: %@, version: %@", end - start, userId, headerSrc, version); //s
}
...
- (NSArray *)stIMDB_selectUserListBySearchStr:(NSString *)searchStr {
    return [self stIMDB_selectUserListBySearchStr:searchStr WithLimit:-1 WithOffset:-1];
}

- (NSInteger)stIMDB_selectUserListTotalCountBySearchStr:(NSString *)searchStr {
    return [[self stIMDB_selectUserListBySearchStr:searchStr] count];
}
