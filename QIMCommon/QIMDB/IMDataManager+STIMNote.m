...
@implementation IMDataManager (STIMNote)
...
- (BOOL)checkExitsMainItemWithQid:(NSInteger)qid WithCId:(NSInteger)cid {
    ...
    if (cid < 1) {
        cid = [[IMDataManager stIMDB_SharedInstance] getMaxQTNoteMainItemCid] + 1;
    }
...
- (BOOL)checkExitsSubItemWithQsid:(NSInteger)qsid WithCsid:(NSInteger)csid {
    __block BOOL existFlag = NO;
    if (csid < 1) {
        csid = [[IMDataManager stIMDB_SharedInstance]getMaxQTNoteSubItemCSid] + 1;
    }
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
