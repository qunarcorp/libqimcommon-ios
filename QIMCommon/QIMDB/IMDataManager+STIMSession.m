...
@implementation IMDataManager (STIMSession)
...
- (long long)stIMDB_insertSessionWithMsgList:(NSDictionary *)msgLists {
    STIMVerboseLog(@"stIMDB_insertSessionWithMsgList : %@", msgLists);
    ...
    for (NSString *key in [msgLists allKeys]) {
        ...
        if (lastMaxMsgTime <= msgTime) {
            ...
            STIMVerboseLog(@"最终比较出来的时间戳是 : %lld", msgTime);
        }
        ...
        if (isConsult) {
            ...
            [[IMDataManager stIMDB_SharedInstance] stIMDB_insertSessionWithSessionId:userId WithUserId:userId WithLastMsgId:userId WithLastUpdateTime:msgTime ChatType:chattype WithRealJid:realJid];
        } else {
            if ([key containsString:@"collection_rbt"]) {
                [[IMDataManager stIMDB_SharedInstance] stIMDB_insertSessionWithSessionId:key WithUserId:[[key componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:msgId WithLastUpdateTime:msgTime ChatType:ChatType_CollectionChat WithRealJid:key];
            } else {
                [[IMDataManager stIMDB_SharedInstance] stIMDB_insertSessionWithSessionId:key WithUserId:[[key componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:msgId WithLastUpdateTime:msgTime ChatType:chattype WithRealJid:key];
            }
        }
...
- (long long)stIMDB_insertGroupSessionWithMsgList:(NSDictionary *)tempGroupDic {
    ...
    for (NSString *groupId in tempGroupDic) {
        if (groupId.length > 0) {
            ...
            [[IMDataManager stIMDB_SharedInstance] stIMDB_insertSessionWithSessionId:groupId WithUserId:[[groupId componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:messageId WithLastUpdateTime:msgDate ChatType:ChatType_GroupChat WithRealJid:groupId];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationOfflineMessageUpdate" object:key userInfo:nil];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationSessionListUpdate" object:nil];
...
- (BOOL)stIMDB_checkSession:(NSString *)sessionId withRealJid:(NSString *)realJid withChatType:(int)chatType {
...
- (void)stIMDB_insertSessionWithSessionId:(NSString *)sessinId
    ...
    BOOL checkExist = [self stIMDB_checkSession:sessinId withRealJid:realJid withChatType:ChatType];
    if (checkExist) {
        [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
...
- (NSArray *)stIMDB_getNotReadSessionList {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        ...
        STIMVerboseLog(@"生成%ld条未读会话列表 耗时 = %f s", result.count, end - start); //s
    }];
    return result;
}
...
- (NSInteger)stIMDB_getAppNotReadCount {
    ...
    //    STIMVerboseLog(@"获取未读数耗时 :%lf", [[STIMWatchDog sharedInstance] escapedTime]);
    STIMVerboseLog(@"");
    return count;
}
