...
@implementation IMDataManager (STIMDBMessage)
...
- (long long)stIMDB_lastestMessageTime {
    ...
    STIMVerboseLog(@"%s-%s 耗时 = %f s", __FILE__, __func__, end - start);
...
- (NSMutableArray *)stIMDB_searchLocalMessageByKeyword:(NSString *)keyWord
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        ...
        while ([reader read]) {
            ...
            NSString *date = [[NSDate stimDB_dateWithTimeIntervalInMilliSecondSince1970:time] stimDB_formattedDateDescription];
...
- (long long)stIMDB_bulkInsertIphoneHistoryGroupJSONMsg:(NSArray *)list WithAtAllMsgList:(NSMutableArray<NSDictionary *> **)atAllMsgList WithNormaleAtMsgList:(NSMutableArray <NSDictionary *> **)normalMsgList {
    ...
    STIMVerboseLog(@"群消息插入本地数据库数量 : %lld", list.count);
    CFAbsoluteTime start = CFAbsoluteTimeGetCurrent();
    ...
    for (NSDictionary *dic in list) {
        ...
        if (msgBody) {
            ...
            platform = [self stIMDB_parserplatForm:[message objectForKey:@"client_type"]];
            ...
            if (lastGroupMsgDate < date.timeIntervalSince1970 - 60 * 2) {
                ...
                [msgDic setObject:[self stIMDB_getTimeSmtapMsgIdForDate:date WithUserId:xmppId] forKey:@"MsgId"];
                ...
                [msgDic setObject:@(STIMMessageDirection_Sent) forKey:@"MsgDirection"];
                [msgDic setObject:@(msec_times-1) forKey:@"MsgDateTime"];
                [msgDic setObject:@(STIMMessageSendState_Success) forKey:@"MsgState"];
                [msgDic setObject:@(STIMMessageRemoteReadStateGroupReaded|STIMMessageRemoteReadStateDidReaded) forKey:@"ReadState"];
            ...
            [msgDic setObject:@(ChatType_GroupChat) forKey:@"ChatType"];
            [msgDic setObject:@(STIMMessageSendState_Success) forKey:@"MsgState"];
            [msgDic setObject:@(STIMMessageRemoteReadStateDidSent) forKey:@"ReadState"];
            ...
            if (direction == STIMMessageDirection_Received) {
                if (msgType == STIMMessageType_NewAt) {
                    NSArray *backupInfoArray = [[STIMJSONSerializer sharedInstance] deserializeObject:backupinfo error:nil];
                    BOOL atMe = NO;
    ...
    long long lastTime = [self stIMDB_insertGroupSessionWithMsgList:resultDic];
    BOOL insertSuccessed = [self stIMDB_bulkInsertMessage:msgList];
    if (updateMsgList.count > 0) {
        [self stIMDB_revokeMessageByMsgList:updateMsgList];
    }
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"插入群消息历史记录%ld条，耗时%fs，成功与否:%d", msgList.count, end - start, insertSuccessed); //s
...
- (NSArray *)stIMDB_bulkInsertIphoneMucPageJSONMsg:(NSArray *)list withInsertDBFlag:(BOOL)flag {
    ...
    for (NSDictionary *dic in list) {
        ...
        if (msgBody) {
            ...
            //翻页消息Check下
            if ([self stIMDB_checkMsgId:msgId] && flag == YES) {
                continue;
            }
            NSString *xmppId = [message objectForKey:@"to"];
            long long msec_times = [[dic objectForKey:@"t"] doubleValue] * 1000;
            ...
            [msgDic setObject:@(ChatType_GroupChat) forKey:@"ChatType"];
            [msgDic setObject:@(STIMMessageSendState_Success) forKey:@"MsgState"];
            [msgDic setObject:@(STIMMessageRemoteReadStateGroupReaded) forKey:@"ReadState"];
            NSData *xmlData = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:nil];
            NSString *xml = [[NSString alloc] initWithData:xmlData encoding:NSUTF8StringEncoding];
            [msgDic setObject:xml?xml:@"" forKey:@"MsgRaw"];
            [msgList addObject:msgDic];
    ...
    if (flag == YES) {
        [self stIMDB_bulkInsertMessage:msgList];
        if (updateMsgList.count > 0) {
            [self stIMDB_revokeMessageByMsgList:updateMsgList];
        }
    }
...
- (NSArray *)stIMDB_bulkInsertIphoneMucPageJSONMsg:(NSArray *)list {
    ...
    return [self stIMDB_bulkInsertIphoneMucPageJSONMsg:list withInsertDBFlag:YES];
}
...
- (long long)stIMDB_bulkInsertHistoryChatJSONMsg:(NSArray *)list {
    STIMVerboseLog(@"插入离线单人JSON消息数量 : %lu", (unsigned long)list.count);
    CFAbsoluteTime start = CFAbsoluteTimeGetCurrent();
    NSMutableDictionary *resultDic = [[NSMutableDictionary alloc] init];
    NSMutableArray *updateMsgList = [[NSMutableArray alloc] init];
    ...
    for (NSDictionary *dic in list) {
        ...
        if (msgBody != nil) {
            ...
            NSInteger platForm = [self stIMDB_parserplatForm:client_type];
            BOOL isConsult = NO;
            if (msgId == nil) {
                msgId = [self UUID];
            }
            ...
            if (lastDate / 1000.0 < date.timeIntervalSince1970 - 60 * 2 || lastDate == 0 && msgType != 2004) {
                ...
                NSMutableDictionary *msgDic = [NSMutableDictionary dictionary];
                [msgDic setObject:[self stIMDB_getTimeSmtapMsgIdForDate:date WithUserId:key] forKey:@"MsgId"];
                [result setObject:[self stIMDB_getTimeSmtapMsgIdForDate:date WithUserId:key] forKey:@"MsgId"];
                ...
                [msgDic setObject:@(STIMMessageRemoteReadStateDidReaded) forKey:@"ReadState"];
                if (isConsult) {
                    if (direction == 0) {
            ...
            if (readFlag == 0) {
                readFlag = STIMMessageRemoteReadStateNotSent;
            } else if (readFlag == 1) {
                readFlag = STIMMessageRemoteReadStateDidSent;
            } else if (readFlag == 3) {
                readFlag = STIMMessageRemoteReadStateDidReaded;
            } else if (readFlag == 4) {
                readFlag = STIMMessageRemoteReadStateDidOperated;
            } else {
                readFlag = STIMMessageRemoteReadStateNotSent;
            }
    ...
    long long lastMaxTime = [self stIMDB_insertSessionWithMsgList:resultDic];
    BOOL success = [self stIMDB_bulkInsertMessage:insertMsgList];
    if (updateMsgList.count > 0) {
        [self stIMDB_revokeMessageByMsgList:updateMsgList];
    }
    if (collectionOriginMsgList.count > 0) {
        [self stIMDB_bulkInsertCollectionMsgWithMsgDics:collectionOriginMsgList];
    }
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"插入单人历史消息%ld条， 耗时 = %f s， 插入成功与否 : %d", insertMsgList.count, end - start, success);
    if (success == YES) {
        return lastMaxTime;
    } else {
...
- (NSArray *)stIMDB_bulkInsertPageHistoryChatJSONMsg:(NSArray *)list
    ...
    return [self stIMDB_bulkInsertPageHistoryChatJSONMsg:list WithXmppId:xmppId withInsertDBFlag:YES];
}
...
#pragma mark - bulkInsertHistoryChatJSONMsg JSOn
        ...
        if (msgBody) {
            ...
            if (isConsult) {
                ...
                if ([fromJid isEqualToString:[self getDbOwnerFullJid]] == YES) {
                    ...
                    key = toJid;
                    direction = STIMMessageDirection_Sent;
                } else {
                    direction = STIMMessageDirection_Received;
                    key = fromJid;
                }
            }
    ...
    if (flag == YES) {
        [self stIMDB_bulkInsertMessage:msgList WithSessionId:xmppId];
    }
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"%s-%s 耗时 = %f s", __FILE__, __func__, end - start);
    return msgList;
...
- (BOOL)stIMDB_bulkInsertMessage:(NSArray *)msgList {
    ...
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"%s-%s 耗时 = %f s", __FILE__, __func__, end - start);
//    STIMVerboseLog(@"插入%ld条消息， 耗时 : %lf", msgList.count, [[STIMWatchDog sharedInstance] escapedTime]);
    return isSuccessed;
}
...
//    [[STIMWatchDog sharedInstance] start];
    if (sesId == nil) {
        return nil;
    }
    CFAbsoluteTime start = CFAbsoluteTimeGetCurrent();
...
//    STIMVerboseLog(@"sql取消息耗时。: %llf", [[STIMWatchDog sharedInstance] escapedTime]);
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"%s-%s 耗时 = %f s", __FILE__, __func__, end - start); //
    return result;
}
...
- (NSInteger)stIMDB_getSumNotReaderMsgCountByXmppIds:(NSArray *)xmppIds {
    ...
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"stIMDB_getSumNotReaderMsgCountByXmppIds:(NSArray *)xmppIds 耗时 = %f s", end - start); //s
    return count;
}
...
- (NSInteger)stIMDB_getNotReaderMsgCountByJid:(NSString *)jid ByRealJid:(NSString *)realJid withChatType:(ChatType)chatType {
    ...
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"耗时 = %f s", end - start); //s
    return count;
}
...
- (NSArray *)stIMDB_getReceiveMsgIdListWithMsgReadFlag:(STIMMessageRemoteReadState)remoteReadState withChatType:(ChatType)chatType withMsgDirection:(STIMMessageDirection)receiveDirection {
    ...
    __block NSMutableArray *resultList = nil;
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        NSString *sql = [NSString stringWithFormat:@"SELECT a.'XmppId', GROUP_CONCAT(MsgId) as msgIdList FROM IM_Message as a WHERE a.ReadState & %d != %d AND a.Direction = %d And a.ChatType = %d GROUP By a.'XmppId';", STIMMessageRemoteReadStateDidReaded, remoteReadState, receiveDirection, chatType];
        DataReader *reader = [database executeReader:sql withParameters:nil];
        while ([reader read]) {
            if (resultList == nil) {
                resultList = [[NSMutableArray alloc] init];
...
- (NSArray *)stIMDB_getNotReadMsgListForUserId:(NSString *)userId ForRealJid:(NSString *)realJid {
    ...
    __block NSMutableArray *result = nil;
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        NSString *sql = [NSString stringWithFormat:@"Select MsgId From IM_Message Where XmppId = '%@' And RealJid = '%@' And ReadState & %d != %d And Direction = %d;", userId, realJid, STIMMessageRemoteReadStateDidReaded, STIMMessageRemoteReadStateDidReaded, STIMMessageDirection_Received];
        DataReader *reader = [database executeReader:sql withParameters:nil];
        while ([reader read]) {
...
- (NSArray *)stIMDB_getNotReadMsgListForUserId:(NSString *)userId {
    ...
    CFAbsoluteTime start = CFAbsoluteTimeGetCurrent();
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        NSString *sql = [NSString stringWithFormat:@"Select MsgId From IM_Message Where XmppId = :XmppId And ReadState & %d != %d And Direction = :MsgDirection;", STIMMessageRemoteReadStateDidReaded, STIMMessageRemoteReadStateDidReaded];
        NSMutableArray *param = [[NSMutableArray alloc] init];
        [param addObject:userId];
        [param addObject:@(STIMMessageDirection_Received)];
        DataReader *reader = [database executeReader:sql withParameters:param];
        while ([reader read]) {
...
- (void)stIMDB_bulkUpdateMessageReadStateWithMsg:(NSArray *)msgs {
    ...
    STIMVerboseLog(@"批量更新消息阅读状态 : %@", msgs);
    CFAbsoluteTime start = CFAbsoluteTimeGetCurrent();
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        ...
        for (NSDictionary *msgInfo in msgs) {
            ...
            STIMMessageRemoteReadState remoteReadState = STIMMessageRemoteReadStateNotSent;
            switch (readFlag) {
                case 0: {
                    remoteReadState = STIMMessageRemoteReadStateNotSent;
                }
                    break;
                case 1: {
                    remoteReadState = STIMMessageRemoteReadStateDidSent;
                }
                    break;
                case 3: {
                    remoteReadState = STIMMessageRemoteReadStateDidReaded;
                }
                    break;
                case 7: {
                    remoteReadState = STIMMessageRemoteReadStateDidOperated;
                }
                    break;
                default:
                    break;
            }
//            STIMVerboseLog(@"MsgId : %@, 阅读状态 : %@", msgId, msgStateLog);
            [paramList addObject:@[@(remoteReadState), msgId]];
        }
        [database executeBulkInsert:sql withParameters:paramList];
    }];
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"批量更新%ld条消息阅读状态 耗时 = %f s", msgs.count, end - start); //
}
...
- (long long)stIMDB_bulkUpdateGroupMessageReadFlag:(NSArray *)mucArray {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase* _Nonnull database) {
        NSString *sql1 = [NSString stringWithFormat:@"select XmppId, LastUpdateTime from IM_Message where XmppId in %@ and ReadState & %d = %d and Direction = %d order by LastUpdateTime asc LIMIT 1;", groupIdParams, STIMMessageRemoteReadStateDidReaded, STIMMessageRemoteReadStateDidReaded, STIMMessageDirection_Received];
        DataReader *reader = [database executeReader:sql1 withParameters:nil];
        STIMVerboseLog(@"离线DB获取群阅读指针参数 ：%@", sql1);
        while ([reader read]) {
            NSString *xmppId = [reader objectForColumnIndex:0];
    ...
    STIMVerboseLog(@"离线DB获取群阅读指针结果 ：%@", dict);
    NSString *sql2 = [NSString stringWithFormat:@"UPDATE IM_Message SET ReadState = (ReadState|%d) WHERE XmppId = :XmppId and LastUpdateTime <= :LastUpdateTime1 and LastUpdateTime >= :LastUpdateTime2;", STIMMessageRemoteReadStateDidReaded];
    ...
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        ...
        STIMVerboseLog(@"离线DB更新群阅读指针参数 ：%@", params);
        [database executeBulkInsert:sql2 withParameters:params];
    }];
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"离线DB更新群阅读指针%ld条数据 耗时 = %f s", mucArray.count, end - start); //s
    
    CFAbsoluteTime start2 = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"离线DB根据群阅读指针更新艾特消息 开始"); //s
    [self stIMDB_clearAtMessageWithGroupReadMarkArray:mucArray];
    CFAbsoluteTime end2 = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"离线DB根据群阅读指针更新艾特消息 耗时 = %f s", end2 - start2); //s

    return maxRemarkUpdateTime;
}

- (void)stIMDB_updateAllMsgWithMsgRemoteState:(int)msgRemoteFlag ByMsgDirection:(int)msgDirection ByReadMarkT:(long long)readMarkT {
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
...
- (void)stIMDB_updateGroupMessageRemoteState:(NSInteger)msgRemoteFlag ByGroupReadList:(NSArray *)groupReadList {
    ...
    STIMVerboseLog(@"在线groupReadList : %@", groupReadList);
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase * _Nonnull database) {
        NSString *sql1 = [NSString stringWithFormat:@"select XmppId, Min(LastUpdateTime) from IM_Message where XmppId in %@ and ReadState & %d != %d;", groupIdParams, STIMMessageRemoteReadStateDidReaded, STIMMessageRemoteReadStateDidReaded, STIMMessageDirection_Received];
        DataReader *reader = [database executeReader:sql1 withParameters:nil];
        STIMVerboseLog(@"在线DB获取群阅读指针参数 ：%@", sql1);
        while ([reader read]) {
            NSString *xmppId = [reader objectForColumnIndex:0];
            NSNumber *lastupdateTime = [reader objectForColumnIndex:1];
            if (xmppId.length > 0) {
    ...
    STIMVerboseLog(@"在线DB获取群阅读指针结果 ：%@", dict);
    NSString *sql2 = [NSString stringWithFormat:@"UPDATE IM_Message SET ReadState = (ReadState|%d) WHERE XmppId = :XmppId and LastUpdateTime <= :LastUpdateTime1 and LastUpdateTime >= :LastUpdateTime2;", STIMMessageRemoteReadStateDidReaded];
    __block long long maxRemarkUpdateTime = 0;
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        NSMutableArray *params = nil;
        ...
        STIMVerboseLog(@"在线DB更新群阅读指针参数 ：%@", params);
        [database executeBulkInsert:sql2 withParameters:params];
    }];
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"在线DB更新群阅读指针%ld条数据 耗时 = %f s", groupReadList.count, end - start); //s
}
...
- (void)stIMDB_updateMsgWithMsgRemoteState:(NSInteger)msgRemoteFlag ByMsgIdList:(NSArray *)msgIdList {
    ...
    [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
        ...
        if (success) {
            STIMVerboseLog(@"更新消息RemoteState状态的参数成功 : %@", paramList);
        }
    }];
    CFAbsoluteTime end = CFAbsoluteTimeGetCurrent();
    STIMVerboseLog(@"更新消息RemoteState状态的参数成功, %ld 耗时 = %f s", msgIdList.count, end - start); //s
}
...
- (void)stIMDB_insertAtMessageWithGroupId:(NSString *)groupId withType:(STIMAtType)atType withMsgId:(NSString *)msgId withMsgTime:(long long)msgTime {
    if (groupId.length > 0 && msgId.length > 0) {
        [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
            ...
            [parames addObject:@(STIMAtMessgaeNotRead)];
...
- (void)stIMDB_UpdateAtMessageReadStateWithGroupId:(NSString *)groupId withReadState:(STIMAtMsgReadState)readState {
    if (groupId.length > 0) {
        if (readState == STIMAtMsgHasReadState) {
            [[self dbInstance] syncUsingTransaction:^(STIMDataBase* _Nonnull database, BOOL * _Nonnull rollback) {
...
- (NSDictionary *)stIMDB_getTotalAtMessageDic {
    ...
    [[self dbInstance] inDatabase:^(STIMDataBase * _Nonnull db) {
        ...
        while ([reader read]) {
            ...
            if ([readState boolValue] == NO) {
                ...
                [resultDic setSTIMSafeObject:groupArray forKey:groupId];
            } else {
