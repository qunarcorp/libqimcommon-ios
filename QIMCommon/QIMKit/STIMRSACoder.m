...
+ (NSString *) RSAForPassword:(NSString *)password
{
    ...
    NSString *md5Key = [STIMRSACoder md5:password];
    
    //
    // step2 rsa
    
    NSString *rsaKey = [STIMRSACoder encryptByRsa:md5Key publicKeyFileName:[STIMNavConfigManager sharedInstance].pubkey];
    
    //
    // step3 url encode key
    
    NSString *key = [STIMRSACoder URLEncodedString:rsaKey];
    return key;
}
...
+ (NSString *)RSAEncrypotoTheData:(NSString *)plainText withPublicKey:(SecKeyRef) publicKey {
    ...
    NSString *encrypotoResult = [NSString stringWithFormat:@"%@",[encryptedData stimDB_base64EncodedString]];
...
+ (NSString *) encryptByRsa:(NSString*)content {
    ...
    NSString *fileName = [STIMNavConfigManager sharedInstance].pubkey;
    int status;
